 ...
 * $Id: cmcustom-10858319-90180751-20140127.txt 234317 2014-03-27 20:53:47Z swehrung $
 * $Revision: 234317 $
 *
 * Version 4.2.0
 *
 ...
 * 03-07-11			yhunt		Updated logic to support test environment per ticket 10066671 
 * 08-15-2012		wbird		Removed li=1010 custom tag from productview tag (10188842)
 * 03-27-2014		swehrung	Updated to cmcustom to add attributes (10858319 )
 *
 */
...
//Note that     __bp = __bp.toString().replace(cmPricePattern, ""); was removed since it caused an error on the client's site add to cart function
...
function DesignCenterCM(elementID){
	cmCreateElementTag(elementID, "OC - Design Center");
}

function StoreWorksheetCM(elementID){
	cmCreateElementTag(elementID, "OC - Store Worksheet");
}

function DesignHelpCM(elementID){
	cmCreateElementTag(elementID, "OC - Design Help");
}

function ProductPageCM(elementID){
	cmCreateElementTag(elementID, "OC - Product Page");
}
function QuickSignInCM(elementID){
	cmCreateElementTag(elementID, "Quick Sign In");
}